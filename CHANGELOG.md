...
- `DecodeAreaRelativeTo` accepted encodings of areas outside the reference area: time ranges ending after it, open-ended ranges inside a closed one, and time differences that overflow uint64 (an overflowing start could wrap to a timestamp before the reference). These are now rejected, as is an `end_from_start` flag on an open range, and an explicit subspace id equal to the reference's is accepted, matching the willow_rs decoder. Found by three upstream EncodeAreaInArea negative vectors.
